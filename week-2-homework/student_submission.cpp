...
void working_thread(const Camera& camera, const std::vector<Sphere>& spheres, Checksum& checksum, int* image_data, const int& beginning,
                    const int& ending)
{
    for (register int y = beginning; y < ending; y++)
    {
        for(register int x = 0; x < WIDTH; x++)
        {
            Vector3 pixel_color(0,0,0);            
            for(register int s = 0; s < NUM_SAMPLES; s++)
            {
...
int main(int argc, char **argv) {
    ...
    int num_samples = HEIGHT/NUM_THREADS;


    for (register int i=0; i < NUM_THREADS; i++)
    {   
        int beginning = i*num_samples;
        int ending = (i+1)*num_samples;
        threads[i] = std::thread(working_thread, std::ref(camera), std::ref(spheres), std::ref(checksum), image_data, beginning, ending);
    }
